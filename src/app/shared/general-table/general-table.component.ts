...
export class GeneralTableComponent implements OnInit, OnDestroy, OnChanges {
    ...
    constructor(
        ...
        const user: any = JSON.parse(us);
        const us2: any = localStorage.getItem('permission')
        let per: any = JSON.parse(us2);
    ...
    doApiCall(ev?: any) {
        ...
        this.apiService.get(this.slug).subscribe((resp: ApiResponse) => {
            ...
            this.loading = false;
            this.apiService.errorToster(err.error['message'], 'Error getting data');
        });
    }
    ...
    onDownload(): void {
        ...
        // const modalRef = this.dail.open(DownloadFileComponent, options);
    ...
    onPageChange(event: any): void {
        ...
        if (this.config.paginationOutside === true) {
            if (this.filters) {
            ...
            const pageEvent = { search_with: this.search.search_with, search_text: this.search.search_text, column: this.sorting.column, direction: this.sorting.direction, offset: (this.offset).toString(), limit: this.limit, page: (pageIndex).toString() };
